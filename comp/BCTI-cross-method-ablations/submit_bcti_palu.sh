...
#SBATCH --chdir=.
#SBATCH --output=./comp/BCTI-cross-method-ablations/bcti_palu-%j.out
#SBATCH --error=./comp/BCTI-cross-method-ablations/bcti_palu-%j.err
...
CONFIG="${CONFIG:-comp/BCTI-cross-method-ablations/config_bcti_palu.yaml}"
OUT_ROOT="${OUT_ROOT:-outputs/50-targets/ablation/cross-methods/bcti-locator-palu-objective}"
...
echo "[cross-methods] variant=bcti-locator-palu-objective facts=${FORGET_IDS[*]} modes=${MODES[*]}"
...
echo "[cross-methods] SUCCESS variant=bcti-locator-palu-objective output=$OUT_ROOT"
