...
#SBATCH --chdir=.
#SBATCH --output=./comp/BCTI-cross-method-ablations/rome_bcti-%j.out
#SBATCH --error=./comp/BCTI-cross-method-ablations/rome_bcti-%j.err
...
CONFIG="${CONFIG:-comp/BCTI-cross-method-ablations/config_rome_bcti.yaml}"
OUT_ROOT="${OUT_ROOT:-outputs/50-targets/ablation/cross-methods/rome-locator-bcti-objective}"
...
echo "[cross-methods] variant=rome-locator-bcti-objective facts=${FORGET_IDS[*]} modes=${MODES[*]}"
...
echo "[cross-methods] SUCCESS variant=rome-locator-bcti-objective output=$OUT_ROOT"
