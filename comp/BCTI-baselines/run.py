...
from ablation import select_ablation_layers  # noqa: E402
...
METHOD_VERSION = "BCTI-ablation-internal"
...
def report_cuda_memory(label: str) -> None:
    ...
    print(
        f"[BCTI-ablation-internal] cuda memory {label}: "
...
def validate_config(cfg: dict, mode: str) -> None:
    ...
    localization_policy = str(ablation.get("localization_policy", "bidirectional_causal"))
    if localization_policy not in {"bidirectional_causal", "forward_causal", "reverse_causal"}:
        raise ValueError(
            "ablation.localization_policy must be bidirectional_causal, "
            "forward_causal, or reverse_causal"
        )
    causal_role = str(ablation.get("causal_score_role", "primary_with_answer_auxiliary"))
    if causal_role not in {
        "primary_with_answer_auxiliary", "primary_only", "localization_and_diagnostic_only"
    }:
        raise ValueError("unsupported ablation.causal_score_role")
    ...
    if causal_role == "primary_with_answer_auxiliary":
        if not 0.0 < auxiliary_weight <= 1.0 or not 0.0 < auxiliary_ratio < 1.0:
            raise ValueError(
                "the full main role requires a positive, causally capped answer auxiliary"
            )
    elif causal_role == "primary_only":
        if auxiliary_weight != 0.0 or auxiliary_ratio != 0.0:
            raise ValueError("primary_only requires zero answer auxiliary weight and ratio")
    else:
        if auxiliary_weight <= 0.0 or auxiliary_ratio != 0.0:
            raise ValueError(
                "localization_and_diagnostic_only requires a positive answer weight "
                "and zero causal-relative cap"
            )
    ...
    elif global_layer_floor_ratio <= 0.0:
        raise ValueError("auxiliary_global_layer_floor_ratio must be positive")
...
def discover_causal_layers(
    ...
    print("[BCTI-ablation-internal] loading TransformerLens model for fit-only tracing", flush=True)
...
def load_causal_layers(
    ...
    layers, layer_summary = select_causal_layers(layer_summary, cfg)
    print(f"[BCTI-ablation-internal] reusing fit-only localization from {source}", flush=True)
    return layers, trace, site_summary, layer_summary
...
def main() -> None:
    parser = argparse.ArgumentParser(description="BCTI-main controlled ablations")
    ...
    if args.localization_source is not None:
        raise ValueError("controlled ablations require independent fit-only localization")
    layers, trace, site_summary, layer_summary = discover_causal_layers(
        base_cfg, splits, cfg["localization"]
    )
    localization_policy = str(
        cfg.get("ablation", {}).get("localization_policy", "bidirectional_causal")
    )
    layers, layer_summary = select_ablation_layers(
        layer_summary, cfg["localization"], localization_policy
    )
    ...
    print(
        f"[BCTI-ablation-internal] localization={localization_policy} "
        f"selected layers: {layers}; loading HF training model",
        flush=True,
    )
    ...
    if not bool(split_audit["all_partitions_directionally_evaluable"]):
        print(
            "[BCTI-ablation-internal] WARNING: direction quotas cannot all be met: "
    ...
    if not bidirectional_trainable:
        print(
            "[BCTI-ablation-internal] WARNING: target is not bidirectionally trainable: "
    ...
    for step in range(1, int(training["max_steps"]) + 1):
            ...
            for fact in facts:
                ...
                causal_role = str(
                    cfg.get("ablation", {}).get(
                        "causal_score_role", "primary_with_answer_auxiliary"
                    )
                )
                ...
                if causal_role == "primary_only":
                    combined_fact_grad = apply_gradient_weights(
                        gradients(causal_loss, parameters), causal_weights
                    )
                    combined_fact_grad = [
                        value * float(causal_training["causal_score_weight"])
                        for value in combined_fact_grad
                    ]
                    auxiliary_pre_mask_grad_norm = 0.0
                    auxiliary_log = dict(zero_log)
                    auxiliary_log["causal_primary_grad_norm"] = gradient_norm(
                        combined_fact_grad
                    )
                    loss = float(causal_training["causal_score_weight"]) * causal_loss
                elif causal_role == "localization_and_diagnostic_only":
                    ...
                    auxiliary_grad = [
                        value * float(causal_training["answer_complement_weight"])
                        * float(answer_direction_cap_fractions[direction])
                        for value in auxiliary_grad
                    ]
                    combined_fact_grad = tier_auxiliary_gradient_by_retain_risk(
                        auxiliary_grad,
                        auxiliary_unsafe_parameter_mask,
                        global_floor_ratio=float(
                            causal_training["auxiliary_global_layer_floor_ratio"]
                    ...
                    applied_norm = gradient_norm(combined_fact_grad)
                    auxiliary_log = dict(zero_log)
                    auxiliary_log.update({
                        "answer_auxiliary_applied_scale": 1.0,
                        "answer_auxiliary_raw_grad_norm": auxiliary_pre_mask_grad_norm,
                        "answer_auxiliary_projected_grad_norm": applied_norm,
                        "answer_auxiliary_applied_grad_norm": applied_norm,
                    })
                    loss = float(causal_training["answer_complement_weight"]) * auxiliary_loss
                else:
        ...
        if should_evaluate:
            ...
            print(
                f"[BCTI-ablation-internal] mode={args.mode} step={step} "
            ...
            if causal_threshold_stop_reached(
                ...
                print(
                    f"[BCTI-ablation-internal] early stop at step={step}: causal score "
            ...
            if (
                ...
                print(
                    f"[BCTI-ablation-internal] early stop at step={step}: no validation improvement "
    ...
    summary = {
        ...
        "causal_localization_is_direction_balanced": (
            localization_policy == "bidirectional_causal"
        ),
        ...
        "ablation_name": str(cfg.get("ablation", {}).get("name", "full_main_reference")),
        "localization_policy": localization_policy,
        "causal_score_role": causal_role,
        "forget_objective": {
            "primary_with_answer_auxiliary": (
                "fixed_baseline_normalized_causal_restore_primary_plus_"
                "capped_bounded_ga_probability_auxiliary"
            ),
            "primary_only": "fixed_baseline_normalized_causal_restore_only",
            "localization_and_diagnostic_only": "bounded_ga_answer_only",
        }[causal_role],
        "auxiliary_forget_loss": str(
            causal_training["auxiliary_forget_loss"]
        ),
        "causal_score_is_primary_forget_objective": (
            causal_role != "localization_and_diagnostic_only"
        ),
        "causal_primary_gradient_norm_guaranteed_above_auxiliary": (
            causal_role == "primary_with_answer_auxiliary"
        ),
        "causal_primary_descent_protected_from_auxiliary_conflict": (
            causal_role == "primary_with_answer_auxiliary"
        ),
        "auxiliary_conflict_policy": (
            "pcgrad_project_auxiliary_against_causal_primary"
            if causal_role == "primary_with_answer_auxiliary"
            else "not_applicable"
        ),
    ...
    print(
        f"[BCTI-ablation-internal] complete mode={args.mode} best_step={best_row['step']} "
