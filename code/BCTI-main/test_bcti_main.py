...
try:
    ...
    if error.name not in {"torch", "transformers", "transformer_lens", "data_utils", "causal_tracing", "config_utils", "tl_pythia_loader"}:
        raise
...
class GradientSurgeryTests(unittest.TestCase):
    ...
    def test_equal_causal_scores_keep_full_layer_weight(self) -> None:
        params = [
            ("gpt_neox.layers.1.attention.query_key_value.lora_a", torch.nn.Parameter(torch.ones(1))),
            ("gpt_neox.layers.3.mlp.dense_h_to_4h.lora_b", torch.nn.Parameter(torch.ones(1))),
        ]
        frame = pd.DataFrame(
