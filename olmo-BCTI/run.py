...
METHOD_VERSION = "OLMo-BCTI-main"
RUNTIME_PATCH_VERSION = "2026-09-12-olmo-eligibility-v3"
...
def validate_config(cfg: dict, mode: str) -> None:
    ...
    if auxiliary_kind != "bounded_ga_probability":
        raise ValueError(
            "BCTI-main requires auxiliary_forget_loss == bounded_ga_probability"
        )
    ...
    if not 0.0 < auxiliary_ratio < 1.0:
        raise ValueError(
            "max_answer_auxiliary_gradient_ratio must be in (0, 1) so causal score remains primary"
        )
    ...
    auxiliary_ratio = float(causal_training["max_answer_auxiliary_gradient_ratio"])
    if not 0.0 < global_layer_floor_ratio <= auxiliary_ratio:
        raise ValueError(
...
def select_causal_layers(
    ...
    for direction in ("forward", "reverse"):
        ...
        if added < per_direction:
            ...
            fallback = ranked[
                ranked["target_causal_candidate"]
                & (ranked[direction_rank] <= max(direction_pool_size, per_direction * 4))
            ].sort_values(
                [coverage_column, column, "target_causal_score", "retain_risk", "layer"],
                ascending=[False, False, False, True, True],
            )
            for value in fallback["layer"].tolist():
                layer = int(value)
                if layer not in selected:
                    selected.append(layer)
                    selection_reason[layer] = f"{direction}_quota_fallback"
                    added += 1
                if added >= per_direction:
                    break
            ranked.loc[fallback.index, "target_causal_floor_relaxed"] = True
        if added < per_direction:
            raise RuntimeError(
                f"target-causal {direction} pool cannot fill its quota even after fallback"
            )
...
def discover_causal_layers(
    ...
    print("[OLMo-BCTI-main] loading OLMo HF model for fit-only tracing", flush=True)
    model, tokenizer, device = load_hf(base_cfg)
    trace_facts = splits["trace_forget"] + splits["trace_retain"]
    trace = run_hf_tracing_multi(
        model,
        tokenizer,
        trace_facts,
    ...
    report_cuda_memory("after releasing OLMo tracing model")
...
def load_causal_layers(
    ...
    layers, layer_summary = select_causal_layers(layer_summary, cfg)
    print(f"[BCTI-main] reusing fit-only localization from {source}", flush=True)
    return layers, trace, site_summary, layer_summary


def _install_olmo_transformers5_compatibility() -> None:
    """Install the OLMo compatibility shim."""
    ...
    legacy_tie_weights = OLMoForCausalLM.tie_weights
    if not getattr(legacy_tie_weights, "_transformers5_compatible", False):
        def compatible_tie_weights(self, *args, **kwargs):
            return legacy_tie_weights(self)

        compatible_tie_weights._transformers5_compatible = True
        OLMoForCausalLM.tie_weights = compatible_tie_weights


def load_hf(base_cfg: dict):
    model_cfg = base_cfg["model"]
    _install_olmo_transformers5_compatibility()
    ...
    for name, value in legacy_config_defaults.items():
...
def _olmo_blocks(model):
    """Return OLMo transformer blocks."""

    try:
        blocks = model.model.transformer.blocks
    except AttributeError as error:
        raise TypeError(
            "expected OLMoForCausalLM.model.transformer.blocks"
        ) from error
    if not blocks:
        raise ValueError("OLMo model has no transformer blocks")
    return blocks


def _hf_site_module(model, spec: CausalInterventionSpec):
    try:
        block = _olmo_blocks(model)[int(spec.layer)]
    except IndexError as error:
        raise ValueError(f"invalid OLMo causal layer: {spec.layer}") from error
    if spec.site == "resid_pre":
        return block, True
    if spec.site == "attn_out":
        return block.attn_out, False
    if spec.site == "mlp_out":
        return block.ff_out, False
    raise ValueError(f"unsupported causal site: {spec.site!r}")


@torch.no_grad()
def run_hf_tracing_multi(
    model, tokenizer, facts, *, sites: list[str],
    corrupt_strategies: list[str], device: str,
) -> pd.DataFrame:
    """Run multi-site Hugging Face tracing."""

    supported = {"resid_pre", "attn_out", "mlp_out"}
    unknown = sorted(set(sites) - supported)
    if unknown:
        raise ValueError(f"unsupported OLMo tracing sites: {unknown}")
    blocks = _olmo_blocks(model)
    rows: list[dict] = []
    for corruption in corrupt_strategies:
        for fact in facts:
            clean_ids = token_ids(tokenizer, fact.prompt, device)
            corrupt_text = corrupt_prompt(fact, strategy=corruption)
            corrupt_ids = token_ids(tokenizer, corrupt_text, device)
            target_id = token_ids(tokenizer, fact.answer, device)[0, 0]
            captured: dict[tuple[int, str], torch.Tensor] = {}
            handles = []
            for layer in range(len(blocks)):
                for site in sites:
                    spec = CausalInterventionSpec(layer, site, corruption)
                    module, is_pre_hook = _hf_site_module(model, spec)
                    key = (layer, site)
                    if is_pre_hook:
                        def capture_pre(_module, inputs, *, key=key):
                            captured[key] = (
                                _first_output_tensor(inputs).detach()[:, -1, :].clone()
                            )
                        handles.append(module.register_forward_pre_hook(capture_pre))
                    else:
                        def capture_out(_module, _inputs, output, *, key=key):
                            captured[key] = (
                                _first_output_tensor(output).detach()[:, -1, :].clone()
                            )
                        handles.append(module.register_forward_hook(capture_out))
            try:
                clean_logits = model(input_ids=clean_ids).logits.float()
            finally:
                for handle in handles:
                    handle.remove()
            expected = len(blocks) * len(sites)
            if len(captured) != expected:
                raise RuntimeError(
                    f"captured {len(captured)} OLMo sites, expected {expected}"
                )
            corrupt_logits = model(input_ids=corrupt_ids).logits.float()
            clean_prob = float(F.softmax(clean_logits[:, -1, :], dim=-1)[0, target_id])
            corrupt_prob = float(F.softmax(corrupt_logits[:, -1, :], dim=-1)[0, target_id])
            for layer in range(len(blocks)):
                for site in sites:
                    spec = CausalInterventionSpec(layer, site, corruption)
                    module, is_pre_hook = _hf_site_module(model, spec)
                    replacement = captured[(layer, site)]
                    if is_pre_hook:
                        def patch_pre(_module, inputs, *, replacement=replacement):
                            values = list(inputs)
                            values[0] = _replace_last_token(
                                _first_output_tensor(inputs), replacement
                            )
                            return tuple(values)
                        handle = module.register_forward_pre_hook(patch_pre)
                    else:
                        def patch_out(
                            _module, _inputs, output, *, replacement=replacement
                        ):
                            return _replace_first_output_tensor(
                                output,
                                _replace_last_token(
                                    _first_output_tensor(output), replacement
                                ),
                            )
                        handle = module.register_forward_hook(patch_out)
                    try:
                        patched_logits = model(input_ids=corrupt_ids).logits.float()
                    finally:
                        handle.remove()
                    patched_prob = float(
                        F.softmax(patched_logits[:, -1, :], dim=-1)[0, target_id]
                    )
                    rows.append(
                        {
                            "id": fact.id,
...
def training_eligibility(
        ...
        records.append(
            {
                ...
                "known_by_configured_standard": known,
                "selection_reason": "base_known" if known else "below_threshold",
            }
        )
    ...
    for direction in ("forward", "reverse"):
        ...
        if len(candidates) < minimum_per_direction:
            raise RuntimeError(
                "forget split is structurally missing BCTI directional probes: "
                f"direction={direction!r}, available={len(candidates)}, "
                f"required={minimum_per_direction}"
            )
        selected_count = sum(index in selected_indices for index in candidates)
        if selected_count >= minimum_per_direction:
            continue
        fallback = sorted(
            (index for index in candidates if index not in selected_indices),
            key=lambda index: (
                -records[index]["base_answer_score"],
                records[index]["id"],
                records[index]["prompt"],
            ),
        )
        for index in fallback[: minimum_per_direction - selected_count]:
            selected_indices.add(index)
            records[index]["selection_reason"] = "direction_score_fallback"

    for index, record in enumerate(records):
        record["selected_for_training"] = index in selected_indices
    selected = [fact for index, fact in enumerate(facts) if index in selected_indices]
...
def causal_parameter_weights(
    ...
    *,
    ...
    selected_layers = sorted(
        {
            int(name.split("model.transformer.blocks.", 1)[1].split(".", 1)[0])
    ...
    parameter_weights = []
    for name, _ in named_parameters:
        layer = int(name.split("model.transformer.blocks.", 1)[1].split(".", 1)[0])
...
def main() -> None:
    print(f"[BCTI-main] runtime_patch={RUNTIME_PATCH_VERSION}", flush=True)
    parser = argparse.ArgumentParser(description="BCTI-main stopping-strategy experiments")
    ...
    print(f"[BCTI-main] selected layers: {layers}; loading HF training model", flush=True)
    model, tokenizer, device = load_hf(base_cfg)
    all_eval_facts = (
        splits["fit_forget"]
        + splits["fit_retain"]
    ...
    if not bool(split_audit["all_partitions_directionally_evaluable"]):
        print(
            "[BCTI-main] WARNING: direction quotas cannot all be met: "
    ...
    if not bidirectional_trainable:
        print(
            "[BCTI-main] WARNING: target is not bidirectionally trainable: "
    ...
    minimum_direction_support = int(
        evaluation["constraints"]["min_eligible_forget_prompts_per_direction"]
    )
    if any(
        count < minimum_direction_support
        for count in validation_direction_counts.values()
    ):
        raise RuntimeError(
            "internal eligibility invariant failed for held-out BCTI probes: "
            f"counts={validation_direction_counts}, "
            f"required_per_direction={minimum_direction_support}"
        )
    ...
    for step in range(1, int(training["max_steps"]) + 1):
            ...
            for fact in facts:
                ...
                combined_fact_grad, auxiliary_log = (
                    combine_primary_and_auxiliary_gradients(
                        causal_grad,
                        auxiliary_grad,
                        primary_weight=float(causal_training["causal_score_weight"]),
                        auxiliary_weight=float(causal_training["answer_complement_weight"]),
                        max_auxiliary_ratio=float(
                            causal_training["max_answer_auxiliary_gradient_ratio"]
                ...
                loss = (
                    float(causal_training["causal_score_weight"]) * causal_loss
                ...
                answer_probability = max(0.0, min(1.0, 1.0 - math.exp(-auxiliary_value)))
                ...
                step_clean_probs.append(float(causal_values["clean_prob"].detach().item()))
                step_corrupt_probs.append(float(causal_values["corrupt_prob"].detach().item()))
                step_patched_probs.append(float(causal_values["patched_prob"].detach().item()))
                step_causal_gaps.append(float(causal_values["corruption_gap"].detach().item()))
                ...
                step_auxiliary_cosines_before.append(
                    float(auxiliary_log["answer_auxiliary_gradient_cosine_before"])
                )
                step_auxiliary_cosines_after.append(
                    float(auxiliary_log["answer_auxiliary_gradient_cosine_after"])
                )
        ...
        if should_evaluate:
            ...
            print(
                f"[BCTI-main] mode={args.mode} step={step} "
            ...
            if causal_threshold_stop_reached(
                ...
                print(
                    f"[BCTI-main] early stop at step={step}: causal score "
            ...
            if (
                ...
                print(
                    f"[BCTI-main] early stop at step={step}: no validation improvement "
    ...
    summary = {
        ...
        "causal_localization_is_direction_balanced": True,
        "causal_layer_candidate_authority": "target_forget_restoration_only",
        "causal_retain_role_in_layer_selection": "secondary_safety_rank_within_target_candidates",
        "causal_layer_score_policy": (
            "bidirectional_harmonic_target_restoration_with_coverage"
        ),
        ...
        "causal_score_is_primary_forget_objective": True,
        "causal_primary_gradient_norm_guaranteed_above_auxiliary": True,
        "causal_primary_descent_protected_from_auxiliary_conflict": True,
    ...
    print(
        f"[BCTI-main] complete mode={args.mode} best_step={best_row['step']} "
