...
#SBATCH --chdir=.
#SBATCH --output=./olmo-BCTI/%j.out
#SBATCH --error=./olmo-BCTI/%j.err
...
CONDA_ENV="${CONDA_ENV:-unlearn-olmo}"
CONFIG="${CONFIG:-olmo-BCTI/config_early_stop.yaml}"
OUT_ROOT="${OUT_ROOT:-outputs/olmo-7b/full/BCTI-main-early-stop}"
...
required_files=(
  models/OLMo-7B/config.json
  models/OLMo-7B/model.safetensors
  "$CONFIG"
  olmo-BCTI/run.py
  olmo-BCTI/lora.py
  olmo-BCTI/metrics.py
  olmo-BCTI/splits.py
  olmo-BCTI/__init__.py
  olmo-BCTI/preflight_splits.py
  olmo-BCTI/test_bcti_main.py
  olmo-BCTI/test_olmo_architecture.py
...
EXPECTED_RUNTIME_PATCH='RUNTIME_PATCH_VERSION = "2026-09-12-olmo-eligibility-v3"'
if ! grep -Fq "$EXPECTED_RUNTIME_PATCH" olmo-BCTI/run.py; then
  echo "[BCTI-main] stale run.py: expected OLMo eligibility patch is missing" >&2
  exit 7
fi
echo "[BCTI-main] verified runtime patch: 2026-09-12-olmo-eligibility-v3"

python -c 'import os, torch, pandas, yaml, transformers, hf_olmo; assert torch.cuda.is_available(), "CUDA is unavailable"; assert torch.cuda.device_count() >= 1, "no visible CUDA device"; print("[BCTI-main] torch:", torch.__version__); print("[BCTI-main] visible_gpus:", torch.cuda.device_count(), "CUDA_VISIBLE_DEVICES=", os.environ.get("CUDA_VISIBLE_DEVICES", "unset")); print("[BCTI-main] gpu0:", torch.cuda.get_device_name(0))'
python -m unittest discover -s olmo-BCTI -p 'test_*.py' -v
python olmo-BCTI/preflight_splits.py --config "$CONFIG"
python olmo-BCTI/run.py --help >/dev/null
if command -v nvidia-smi >/dev/null 2>&1; then
  nvidia-smi --query-gpu=index,name,memory.total,memory.free --format=csv,noheader
fi

echo "[BCTI-main] launch=$launch_kind facts=${RUN_IDS[*]} modes=${RUN_MODES[*]} localization=fresh-per-fact"
  ...
  for mode in "${RUN_MODES[@]}"; do
    ...
    if [[ -e "$out_dir" ]] && [[ -n "$(find "$out_dir" -mindepth 1 -maxdepth 1 -print -quit 2>/dev/null)" ]]; then
      if [[ -f "$out_dir/summary.json" ]]; then
        echo "[BCTI-main] already complete; skipping $forget_id/$mode"
        continue
      fi
      archive_dir="${out_dir}.partial-${SLURM_JOB_ID:-manual}-$(date +%s)"
      mv -- "$out_dir" "$archive_dir"
      echo "[BCTI-main] archived incomplete output: $out_dir -> $archive_dir" >&2
    fi
    mkdir -p "$out_dir"
    echo "[BCTI-main] starting forget=$forget_id mode=$mode output=$out_dir"

    python -u olmo-BCTI/run.py \
      --config "$CONFIG" \
      --forget-id "$forget_id" \
      --mode "$mode" \
      --out-dir "$out_dir"
...
if summary.get("version") != "OLMo-BCTI-main":
...
PY
    echo "[BCTI-main] finished forget=$forget_id mode=$mode"
  done
done
